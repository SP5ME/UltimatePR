...
func RunSetup(ctx context.Context, listen, configPath string, log *slog.Logger) error {
	...
	mux.HandleFunc("POST /api/setup", func(w http.ResponseWriter, r *http.Request) {
		...
		if q.Password != q.Confirm || len(q.Password) < 8 || len(q.Password) > 128 {
			http.Error(w, "Hasła muszą być identyczne i mieć 8–128 znaków", 400)
			return
		}
...
const setupHTML = `<!doctype html><html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>UltimatePR — pierwsze uruchomienie</title><style>
body{font:16px system-ui;background:#eef3ee;color:#17251b;margin:0}.box{max-width:720px;margin:5vh auto;background:#fff;padding:28px;border:1px solid #bdc9be;border-radius:12px}label{display:block;margin:13px 0 5px}input,select,button{font:inherit;padding:10px;border:1px solid #aab8ac;border-radius:7px}input,select{width:100%;box-sizing:border-box}button{background:#1e713b;color:white;margin-top:18px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:12px}.muted{color:#59685d}.error{color:#a11;white-space:pre-wrap}@media(max-width:600px){.grid{display:block}}</style></head><body><main class="box"><h1>Konfigurator UltimatePR</h1><p class="muted">Utwórz nową konfigurację albo wczytaj wcześniej pobraną kopię konfiguracji.</p>
...
<form id="setup"><label>Tryb pracy</label><select name="mode"><option value="station">Tylko stacja</option><option value="station-node-bbs">Stacja + NODE + BBS</option></select><div class="grid"><div><label>Znak wywoławczy</label><input name="callsign" required maxlength="6"></div><div><label>Lokator Maidenhead</label><input name="locator" required placeholder="KO02AB"></div></div><label>QTH</label><input name="qth" required><label>Język</label><select name="language"><option value="pl">Polski</option><option value="en">English</option></select><div class="grid"><div><label>SSID stacji</label><input name="station_ssid" type="number" min="0" max="15" value="0"></div><div><label>SSID NODE</label><input name="node_ssid" type="number" min="0" max="15" value="7"></div><div><label>SSID BBS</label><input name="bbs_ssid" type="number" min="0" max="15" value="8"></div></div><label>Użytkownik panelu</label><input name="username" value="admin" required><div class="grid"><div><label>Hasło</label><input name="password" type="password" minlength="8" required></div><div><label>Powtórz hasło</label><input name="confirm_password" type="password" minlength="8" required></div></div><button>Zapisz i uruchom UltimatePR</button></form><p id="msg" class="error"></p></main><script>
const msg=document.querySelector('#msg');async function send(url,body,headers){msg.textContent='Zapisywanie…';const r=await fetch(url,{method:'POST',body,headers});if(!r.ok){msg.textContent=await r.text();return}msg.style.color='#176b36';msg.textContent='Zapisano. UltimatePR uruchomi się ponownie.';setTimeout(()=>location.href='/',4000)}
document.querySelector('#setup').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),o=Object.fromEntries(f);['station_ssid','node_ssid','bbs_ssid'].forEach(k=>o[k]=Number(o[k]));send('/api/setup',JSON.stringify(o),{'Content-Type':'application/json'})};document.querySelector('#import').onsubmit=e=>{e.preventDefault();send('/api/setup/import',new FormData(e.target))};</script></body></html>`
