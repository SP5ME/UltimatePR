...
const setupJS = `const msg=document.querySelector('#msg');const mode=document.querySelector('select[name="mode"]');const advancedFields=[...document.querySelectorAll('[data-mode-group="advanced"]')];function syncModeFields(){const advanced=mode&&mode.value==='station-node-bbs';for(const el of advancedFields)el.hidden=!advanced}async function send(url,body,headers){msg.className='error';msg.textContent='Zapisywanie�';const r=await fetch(url,{method:'POST',body,headers});if(!r.ok){msg.textContent=await r.text();return}msg.className='success';msg.textContent='Zapisano. UltimatePR uruchomi si� ponownie.';setTimeout(()=>location.href='/',4000)}
if(mode){mode.addEventListener('change',syncModeFields);syncModeFields()}document.querySelector('#setup').onsubmit=e=>{e.preventDefault();const f=new FormData(e.target),o=Object.fromEntries(f);['station_ssid','node_ssid','bbs_ssid'].forEach(k=>o[k]=Number(o[k]));send('/api/setup',JSON.stringify(o),{'Content-Type':'application/json'})};document.querySelector('#import').onsubmit=e=>{e.preventDefault();send('/api/setup/import',new FormData(e.target))};`
