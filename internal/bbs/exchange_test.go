...
func TestTwoBBSForwardingAndASCII(t *testing.T) {
	...
	m, err := a.Send("P", "SP5AAA", "SP5ME@SP5BBB-8", "Zażółć", "Wiadomość dla Łukasza")
	if err != nil {
		t.Fatal(err)
	}
	...
	defer cancel()
	remote := &Server{Node: "SP5BBB-8", Store: b}
	go remote.runForwardListener(ctx, ln)
	peer := ForwardPeer{ID: "bbs-b", Enabled: true, Transport: "telnet", Host: "127.0.0.1", Port: uint16(ln.Addr().(*net.TCPAddr).Port), PrivateRoutes: []string{"SP5BBB"}}
	if err = PrepareQueues(a, []ForwardPeer{peer}, 10); err != nil {
		t.Fatal(err)
	}
	f := &Forwarder{Store: a, LocalCall: "SP5AAA-8", MaxMessages: 10, ConnectTimeout: time.Second, SessionTimeout: time.Second}
	if err = f.forwardPeer(ctx, peer); err != nil {
		t.Fatal(err)
	}
	got := b.List("SP5ME", false)
	if len(got) != 1 || got[0].BID != m.BID {
	...
	if len(b.List("SP5ME", false)) != 1 {
		t.Fatal("duplicate BID imported")
	}
}
